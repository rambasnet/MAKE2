...
FileInfoQueue = Queue.Queue()
ThumbnailQueue = Queue.Queue()
SubDirQueue = Queue.Queue()
ScanDone = False
StartTime = 0
DirCount = 0
FilesCount = 0
...
[wxID_DLGSCANPROGRESS, wxID_DLGSCANPROGRESSBTNCANCEL, 
 ...
 wxID_DLGSCANPROGRESSLBLSCANSTATUS, wxID_DLGSCANPROGRESSLBLSTARTTIME, 
 wxID_DLGSCANPROGRESSLBLTOTALDIR, wxID_DLGSCANPROGRESSPANSCANSTATUS, 
 wxID_DLGSCANPROGRESSSTATICTEXT1, wxID_DLGSCANPROGRESSSTATICTEXT2, 
 wxID_DLGSCANPROGRESSSTATICTEXT3, wxID_DLGSCANPROGRESSSTATICTEXT4, 
 wxID_DLGSCANPROGRESSSTATICTEXT5, 
...
class dlgScanProgress(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        wx.Dialog.__init__(self, id=wxID_DLGSCANPROGRESS,
              name=u'dlgScanProgress', parent=prnt, pos=wx.Point(826, 333),
              size=wx.Size(448, 310), style=0, title=u'Scan Status...')
        self._init_utils()
        self.SetClientSize(wx.Size(440, 279))
        ...
        self.panScanStatus = wx.Panel(id=wxID_DLGSCANPROGRESSPANSCANSTATUS,
              ...
              size=wx.Size(408, 216), style=wx.TAB_TRAVERSAL)
        ...
        self.staticText3 = wx.StaticText(id=wxID_DLGSCANPROGRESSSTATICTEXT3,
              ...
              parent=self.panScanStatus, pos=wx.Point(16, 192), size=wx.Size(82,
              13), style=wx.ALIGN_RIGHT)
        ...
        self.btnOK = wx.Button(id=wxID_DLGSCANPROGRESSBTNOK, label=u'&OK',
              name=u'btnOK', parent=self, pos=wx.Point(176, 240),
        ...
        self.btnCancel = wx.Button(id=wxID_DLGSCANPROGRESSBTNCANCEL,
              ...
              pos=wx.Point(176, 240), size=wx.Size(88, 24), style=0)
        ...
        self.staticText1 = wx.StaticText(id=wxID_DLGSCANPROGRESSSTATICTEXT1,
              ...
              parent=self.panScanStatus, pos=wx.Point(8, 160), size=wx.Size(92,
              13), style=0)
    ...
    def OnUpdate(self, evt):
        ...
        self.lblKnownFilesCount.SetLabel(PlatformMethods.Convert(evt.KnownFilesCount))
    ...
    def UpdateEvidence(self):
        ...
        Globals.EvidencesDict[self.EvidenceID] = {'DisplayName':'Evd1', 'Location':self.rootDir, 'NewLocation':""}
        tuple = (self.EvidenceID, 'Evd1', self.rootDir, 'No comment', StartTime, DirCount,
            FilesCount, TotalImages, StartTime, time.time())
...
class MACScanThread(threading.Thread):
    ...
    def HandleRecursiveZipFile(self, unzippedPath, dirPath, fileName):
        ...
        NewRelativeRoot = "%d%s"%(self.UnzipFileNameDict[unzipFileName], unzipFileName)
        extractRootPath = os.path.join(Globals.CasePath, 'Evd1Unzip')
        ...
        for name in unzip.namelist():
            ...
            FileInfoQueue.put((Name, DirPath, Extension, MimeType, Category, Description, Size, Created, cDate, cMonth,
                Modified, mDate, mMonth, Accessed, aDate, aMonth,  MD5, KnownFile, NewPath))
            ...
            if MimeType.find("image/") == 0:
                TotalImages += 1
                try:
                    Thumbnail = self.GetThumbnail(extractFilePath).GetData()
                except Exception, msg:
                ...
                ThumbnailQueue.put((DirPath, Name, Thumbnail))
        ...
        for aPath in ZipPath:
            SubDirQueue.put((aPath, cPickle.dumps(list(ZipPath[aPath]))))
            DirCount += len(ZipPath[aPath])
    ...
    def SendEvent(self):
        evt = UpdateLabelEvent(elapsedTime = self.ElapsedTime,  KnownFilesCount = KnownFilesCount,
            filesCount = FilesCount, totalDir = DirCount,
...
class WriteThread1(threading.Thread):
    def __init__(self, win, rootDir):
        threading.Thread.__init__(self)
        self.win = win
        #self.StartTime = startTime
    ...
    def run(self):
        ...
        while True:
            #print 'second thread'
            #if ScanDone and FileInfoQueue.empty() and SubDirQueue.empty():
            if ScanDone and FileInfoQueue.empty():
                break
            ...
            try:
                fileInfo = FileInfoQueue.get_nowait()
                self.dbFileSystem.ExecuteMany(self.query, [fileInfo])
            except:
                pass
            """
            
            try:
                subDirInfo = SubDirQueue.get_nowait()    
                self.dbFileSystem.ExecuteMany(self.dirListQuery, [subDirInfo])
            except:
                pass
            """
        ...
        """
        if WriteThread2Done == True:
            self.ElapsedTime = CommonFunctions.ConvertSecondsToDayHourMinSec(time.time() - StartTime)
            evt = UpdateLabelEvent(elapsedTime = self.ElapsedTime, KnownFilesCount = KnownFilesCount,
                totalDir = DirCount, filesCount=FilesCount, scanStatus = "Done Scanning!")
    ...
    def UpdateEvidence(self, location):
        ...
        tuple = (self.EvidenceID, 'Evd1', location, 'No comment', StartTime, DirCount,
            FilesCount, TotalImages, StartTime, time.time())
...
class WriteThread2(threading.Thread):
    def __init__(self, win, rootDir):
        threading.Thread.__init__(self)
        self.win = win
        #self.StartTime = startTime
    ...
    def run(self):
        ...
        while True:
            #print 'second thread'
            if ScanDone and Thumbnail.empty():
                break
            ...
            try:
                thumbnailInfo = ThumbnailQueue.get_nowait()
                self.dbImage.ExecuteMany(self.imageQuery, [thumbnailInfo])
            except:
                pass
