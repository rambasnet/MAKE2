...
import wx,os,sys,time,calendar,math
...
def create(prnt):
    return MDIChildTimeline(prnt)


# Create a new frame class, derived from the wxPython Frame.
class MDIChildTimeline(wx.MDIChildFrame, listmix.ColumnSorterMixin):
    def __init__(self, prnt):#as a frame, it needed title also
        ...
        self.minT = -1
        self.maxT = -1
        self.SelectedFileType = "All File Types"
        ...
        for key in Constants.TimelineKeys:
            ...
            self.TimelinesDict[key]['PixelSizeArrays'] = []
            self.TimelinesDict[key]['TotalPixels'] = 0
            self.TimelinesDict[key]['Selected'] = True
            self.TimelinesDict[key]['TimePerPixel'] = 0
            self.TimelinesDict[key]['PixelPerTime'] = 0
            self.TimelinesDict[key]['TimeSpan'] = 0
            self.TimelinesDict[key]['Selections'] = []
            self.TimelinesDict[key]['StartSelection'] = time.time()
            self.TimelinesDict[key]['EndSelection'] = time.time()
        
        self.ClearSelections()
        self.initdone = 1
        self.draw()
        self.SelectedFileList = []
        self.fileCategoryView = FileCategoryView(self, self.treeCatList, Globals.MimeTypeSet)
    ...
    def CreateTimelinesPanel(self):
        ...
        self.staticText3 = wx.StaticText(id=-1, label='Time Span:', name='staticText3', parent=self.panMainTimelines,
    ...
    def _init_coll_popupMenuFile_Items(self, parent):
        ...
        parent.Append(help='',
              id=viewId,
              kind=wx.ITEM_NORMAL, text='Open File in Hex Viewer')
        
        openId = wx.NewId()
        parent.Append(help='',
              id=openId,
              kind=wx.ITEM_NORMAL, text='Open File In Native Viewer')
    ...
    def GetSelectedFileInfo(self):
        #(DirPath||'%s'||Name), Size, Created, Modified, Accessed, Category, MimeType, Description, MD5, KnownFile
        self.fileObject = None
        for alist in self.SelectedFileList:
            if alist[0] == self.selectedFilePath:
                self.fileObject = Classes.File()
                self.fileObject.DirPath = os.path.dirname(self.selectedFilePath)
                self.fileObject.Name = os.path.basename(self.selectedFilePath)
                self.fileObject.Size = alist[1]
                self.fileObject.Created = alist[2]
                self.fileObject.Modified = alist[3]
                self.fileObject.Accessed = alist[4]
                self.fileObject.Category = alist[5]
                self.fileObject.MimeType = alist[6]
                self.fileObject.Description = alist[7]
                self.fileObject.MD5 = alist[8]
                self.fileObject.KnownFile = alist[9]
                break
    ...
    def CreateFileList(self):
        ...
        self.staticTextPage = wx.StaticText(id=wx.NewId(),
              ...
              pos=wx.Point(16, 73), size=wx.Size(28, 13), style=0)
        self.staticTextPage.SetFont(wx.Font(8, wx.SWISS, wx.NORMAL, wx.BOLD,
              False, 'Tahoma'))
              
        self.lblFileListCount = wx.StaticText(id=wx.NewId(),
              label='of 0:  Showing 0 of 0 File', name='lblFileListCount', parent=self.panFileList,
              pos=wx.Point(130, 73), size=wx.Size(35, 13), style=0)
        ...
        self.choiceFileListPageNum = wx.Choice(choices=['0'],
            ...
            parent=self.panFileList, pos=wx.Point(56, 65), size=wx.Size(64,
            21), style=0)
        self.choiceFileListPageNum.Bind(wx.EVT_CHOICE, self.OnChoiceFileListPageNumChoice,
          id=choiceId)
              
        self.lblListViewTitle = wx.StaticText(id=-1,
              label=u'', name=u'self.lblListViewTitle', parent=self.panFileList,
              pos=wx.Point(8, 33), size=wx.Size(26, 16), style=0)
        self.lblListViewTitle.SetForegroundColour(wx.Colour(0, 0, 187))
        self.lblListViewTitle.SetFont(wx.Font(8, wx.SWISS, wx.NORMAL, wx.BOLD,
              False, u'Tahoma'))
        ...
        listID = wx.NewId()
        self.listFilesDetails = CustomListCtrl(self.panFileList, listID, pos=wx.Point(8,97), size=wx.Size(380, 91),
    ...
    def CalculateTimeOrbs(self):
        ...
        for key in Constants.TimelineKeys:
            if self.TimelinesDict[key]['Selected']:
                ...
                columnName = self.GetColumnName(key)
                query = "select distinct(%s) from Evidence1 where %s >= %s and %s <= %s"%(columnName, columnName, self.TimelinesDict[key]['FromTime'], columnName, self.TimelinesDict[key]['ToTime']) 
                #print query
                rows = self.db.FetchAllRows(query)
                ...
                for row in rows:
                    ...
                    index = int((row[0] - self.TimelinesDict[key]['FromTime']) * self.TimelinesDict[key]['PixelPerTime'])
                    if (index >= 0 and index <= self.TimelinesDict[key]['TotalPixels']):
                        ...
                        query = "select count(*) from Evidence1 where %s=%s"%(columnName, row[0])
                        #print query
                        
                        row = self.db.FetchOneRow(query)
                        if row:
                            self.TimelinesDict[key]['PixelSizeArrays'][index] = int(row[0]) #fileObj.Size #self.FileObjectList[o][3]
                        else:
                            self.TimelinesDict[key]['PixelSizeArrays'][index] = 0
    ...
    def OnViewClick(self, event):
        ...
        self.SelectedFileType = "All File Types"
        self.AddPagesMultipleSelection()
    ...
    def OnMouseMove(self, evt):
        ...
        if self.dragShape:
            ...
            if (timeline > -1):
                ...
                key = Constants.TimelineKeys[timeline]
                if (evt.m_x < 1 or evt.m_x > self.TimelinesDict[key]['TotalPixels']+self.edge+1):
                    self.lblMouseAt.SetLabel(" ")
                    evt.Skip()
                    return
                else:
    ...
    def IsFileSelected(self):
        ...
        if self.index >=0:
            selectedFilePath = PlatformMethods.Encode(self.listFilesDetails.GetItem(self.index).GetText())
            self.selectedFilePath = self.GetFilePath(selectedFilePath)
            return True
        else:
    ...
    def GetFilePath(self, fullFilePath):
        filePath = ""
        self.fileInfo = None
        for fileinfo in self.SelectedFileList:
            if fileinfo[0] == fullFilePath:
                if fileinfo[10]:
                    filePath = os.path.join(Globals.CasePath, fileinfo[10])
                else:
                    filePath = fileinfo[0]
                ...
                self.fileInfo.insert(0, os.path.basename(fullFilePath))
                self.fileInfo[1] = os.path.dirname(fullFilePath)
                for i in range(5):
                    self.fileInfo.insert(len(self.fileInfo)-1, 'None')
                
                #print self.fileInfo
                """
                "SELECT (DirPath||'%s'||Name), Size, Created, Modified, Accessed, Category, MimeType, Description, MD5,"%(os.path.sep)
                KnownFile, NewPath, Export  FROM Evidence1 where %s=%s limit %d offset %d;
                """
                """Required in this format for hex viewer
    ...
    def OpenFile(self):
        ...
        try:
            os.startfile(PlatformMethods.Decode(self.selectedFilePath))
        except Exception, value:
            CommonFunctions.ShowErrorMessage(self, str(value), error=True)
            
        """
        fileExtension = self.selectedFilePath[self.selectedFilePath.rfind('.'):]
        fileType = wx.TheMimeTypesManager.GetFileTypeFromExtension(fileExtension)
        cmd = ""
        if fileType:
            mimeType = fileType.GetMimeType() or ""
            if mimeType:
                cmd = fileType.GetOpenCommand(self.selectedFilePath, mimeType)
                if cmd:
                    if cmd.find('WX_DDE#') >= 0:
                        cmd = cmd.replace('WX_DDE#', "")
                        cmd = '%s" "%s"'%(cmd, self.selectedFilePath)
        if cmd:
            try:
                os.system('start '+cmd)
            except Exception, value:
    ...
    def OnListFilesDoubleClick(self, event):
        if self.IsFileSelected():
            #self.OpenFile()
            self.OnPopupMenuFileMenuitemviewfileMenu(event)
        event.Skip()
    ...
    def GetPathName(self, item, treeCtrl):
        #pathName = []
        return self.GetTreeItemText(item)
        """
    ...
    def OnTreeCatListTreeSelChanged(self, event):
        ...
        if item == self.root:
            ...
            self.SelectedFileType = self.GetTreeItemText(item)
            
            """
            fileIndex = 0
            #category = self.SelectedFileType.split("/")
            for row in self.SelectedFileList:
                if row[6] == self.SelectedFileType:
                    self.CreateListItem(row, fileIndex)
                    fileIndex += 1
            
            self.TotalFiles = fileIndex
            """
    ...
    def AddPagesMultipleSelection(self):
        #filesCount = 0
        
        
        query = """SELECT count(*) From Evidence1 where ("""
        where = ""
        oneSelection = False
        for key in Constants.TimelineKeys:
            if self.TimelinesDict[key]['Selected']:
                ...
                columnName = self.GetColumnName(key)
                if where:
                    where += " or (%s>=%s and %s<=%s)"%(columnName, self.TimelinesDict[key]['StartSelection'], columnName, self.TimelinesDict[key]['EndSelection'])
                else:
                    where += " (%s>=%s and %s<=%s)"%(columnName, self.TimelinesDict[key]['StartSelection'], columnName, self.TimelinesDict[key]['EndSelection'])
        ...
        if self.SelectedFileType != "All File Types":
            where += " and (MimeType = '%s')"%(self.SelectedFileType)
        ...
        #print filesCount
        self.TotalPages = int(math.ceil(self.TotalFiles/float(Constants.MaxObjectsPerPage)))
        self.AddPageNumbersToChoice(self.TotalPages)
        self.choiceFileListPageNum.SetSelection(0)
    
    
    def GetSelectedFiles(self, page=0):
        self.InitializeFileListStuff()
        fileIndex = 0
        self.ListViewTitle = ""
        #for fileObj in self.FileObjectList:
               
        query = """SELECT DirPath||'%s'||Name, Size, Created, Modified, Accessed, Category, MimeType,
            Description, MD5, KnownFile, NewPath, Export FROM Evidence1 where ("""%(os.path.sep)
        ...
        for key in Constants.TimelineKeys:
            if self.TimelinesDict[key]['Selected']:
                ...
                if self.ListViewTitle == "":
                    self.ListViewTitle = "%s [%s - %s]"%(key, DisplayDate(self.TimelinesDict[key]['StartSelection']), DisplayDate(self.TimelinesDict[key]['EndSelection']))
                else:
                    self.ListViewTitle += " - %s [%s - %s]"%(key, DisplayDate(self.TimelinesDict[key]['StartSelection']), DisplayDate(self.TimelinesDict[key]['EndSelection']))
                #timeIndex = Constants.TimelineKeys.index(key)
                columnName = self.GetColumnName(key)
        ...
        query += "%s limit %d offset %d"%(where, Constants.MaxObjectsPerPage, page)
        #print query
        rows = self.db.FetchAllRows(query)
        ...
        self.SelectedFileList = self.FileInfo.values()
    ...
    def AddSliderOnDateFiles(self):
        ...
        SHA224 text default 'NULL',
        SHA256 text default 'NULL',
        SHA384 text default 'NULL',
        SHA512 text defaul 'NULL',
        NewPath text default 'NULL',
        KnownFile Number)
        
        listItem.append(os.path.join((fileInfo.DirPath), (fileInfo.Name)))
        ...
        listItem.append(PlatformMethods.Decode(fileInfo.MimeType))
        listItem.append(PlatformMethods.Decode(fileInfo.Description))
        ...
        columnName = self.GetColumnName(self.TimelineID)
        query = "SELECT COUNT(*) FROM Evidence1 where %s=%s;"%(columnName, self.DateSliderOn)
        row = self.db.FetchOneRow(query)
        self.TotalFiles = int(row[0])
        self.TotalPages = int(math.ceil(self.TotalFiles/float(Constants.MaxObjectsPerPage)))
        self.AddPageNumbersToChoice(self.TotalPages)
        self.choiceFileListPageNum.SetSelection(0)
        self.SliderUpdateFilesInListView(0)
    


    def SliderUpdateFilesInListView(self, page):
        self.InitializeFileListStuff()
        fileIndex = 0
        query = "SELECT (DirPath||'%s'||Name), Size, Created, Modified, Accessed, Category, MimeType, Description, MD5,"%(os.path.sep)
        query += "KnownFile, NewPath, Export  FROM Evidence1 where %s=%s limit %d offset %d;"%(self.GetColumnName(self.TimelineID), self.DateSliderOn,  Constants.MaxObjectsPerPage, page)
        rows = self.db.FetchAllRows(query)
        for row in rows:
            ...
            #self.SelectedFileList.append(row)
            self.CreateListItem(row, fileIndex)
            fileIndex += 1
    ...
    def AddItemsToListView(self):
        ...
        #print len(items)
        for key, data in self.FileInfo.items():
        ...
        #try:
            ...
            index = self.listFilesDetails.InsertImageStringItem(sys.maxint, PlatformMethods.Decode(fullFilePath), self.IconDict[key])
            self.listFilesDetails.SetStringItem(index, 1, CommonFunctions.ConvertByteToKilobyte(data[1]))
            self.listFilesDetails.SetStringItem(index, 2, CommonFunctions.GetShortDateTime(data[2]))
            self.listFilesDetails.SetStringItem(index, 3, CommonFunctions.GetShortDateTime(data[3]))
            self.listFilesDetails.SetStringItem(index, 4, CommonFunctions.GetShortDateTime(data[4]))
            ...
            if data[9] == 1:
                self.listFilesDetails.SetStringItem(index, 9, 'Yes')
            else:
                self.listFilesDetails.SetStringItem(index, 9, 'No')
                
            if data[11] == 1:
                self.listFilesDetails.SetStringItem(index, 10, 'Yes')
            else:
                self.listFilesDetails.SetStringItem(index, 10, 'No')
            ...
            self.listFilesDetails.SetStringItem(index, 11, PlatformMethods.Decode(newPath))
        ...
        self.lblListViewTitle.SetLabel(self.ListViewTitle + " - " + self.SelectedFileType)
        self.lblFileListCount.SetLabel("of %s:  Showing %s of %s %s"%(CommonFunctions.GetCommaFormattedNumber(self.TotalPages), CommonFunctions.GetCommaFormattedNumber(len(self.FileInfo)), CommonFunctions.GetCommaFormattedNumber(self.TotalFiles), value))
        self.SetCursor(wx.STANDARD_CURSOR)             
        #wx.EndBusyCursor()
                        
    def CreateListItem(self, listItem, fileIndex):
        self.FileInfo[fileIndex] = tuple(listItem)
        #print self.FileInfo
        iconFound = False
        NoLog = wx.LogNull()
        if not (listItem[0].rfind('.') == -1):
            fileExtension = listItem[0][listItem[0].rfind('.'):]
            fileType = wx.TheMimeTypesManager.GetFileTypeFromExtension(fileExtension)
            if fileType:
    ...
    def OnChoiceFileListPageNumChoice(self, event):
        wx.BeginBusyCursor()
        page = (int(self.choiceFileListPageNum.GetStringSelection())-1)*Constants.MaxObjectsPerPage
        if self.DateSliderOn > 0:
            self.SliderUpdateFilesInListView(page)
        else:
            self.GetSelectedFiles(page)
