...
class MDIChildTimeline(wx.MDIChildFrame, listmix.ColumnSorterMixin):
    def __init__(self, prnt, TimelinesDict):#as a frame, it needed title also
        ...
        """
        Constants.TimelineKeys = ['Created', 'Modified', 'Accessed']
        #for a in (0, 1, 2): #Created Time, Modified Time, Accessed Time
        for a in ("Created", "Modified", "Accessed"): #Created Time, Modified Time, Accessed Time
            self.TimelinesDict[a] = {}
            self.TimelinesDict[a]['MaxTime'] = -1
            self.TimelinesDict[a]['MinTime'] = -1
            self.TimelinesDict[a]['PixelArrays'] = []
            self.TimelinesDict[a]['PixelSizeArrays'] = []
            self.TimelinesDict[a]['TotalPixels'] = 0
            self.TimelinesDict[a]['Selected'] = True
            self.TimelinesDict[a]['FromTime'] = 0
            self.TimelinesDict[a]['ToTime'] = 0
            self.TimelinesDict[a]['TimePerPixel'] = 0
            self.TimelinesDict[a]['PixelPerTime'] = 0
            self.TimelinesDict[a]['TimeSpan'] = 0
            self.TimelinesDict[a]['Selections'] = []
            self.TimelinesDict[a]['StartSelection'] = time.time()
            self.TimelinesDict[a]['EndSelection'] = time.time()
        """
        ...
        for file in self.FileObjectList:
            ...
            if (self.TimelinesDict['Created']['MinTime'] == -1 or self.TimelinesDict['Created']['MinTime'] > ctime):
                self.TimelinesDict['Created']['MinTime'] = ctime
                
            if (self.TimelinesDict['Created']['MaxTime'] == -1 or self.TimelinesDict['Created']['MaxTime'] < ctime):
                self.TimelinesDict['Created']['MaxTime'] = ctime
            
            if (self.TimelinesDict['Modified']['MinTime'] == -1 or self.TimelinesDict['Modified']['MinTime'] > mtime):
                self.TimelinesDict['Modified']['MinTime'] = mtime
                
            if (self.TimelinesDict['Modified']['MaxTime'] == -1 or self.TimelinesDict['Modified']['MaxTime'] < mtime):
                self.TimelinesDict['Modified']['MaxTime'] = mtime
                
            if (self.TimelinesDict['Accessed']['MinTime'] == -1 or self.TimelinesDict['Accessed']['MinTime'] > atime):
                self.TimelinesDict['Accessed']['MinTime'] = atime
                
            if (self.TimelinesDict['Accessed']['MaxTime'] == -1 or self.TimelinesDict['Accessed']['MaxTime'] < atime):
                self.TimelinesDict['Accessed']['MaxTime'] = atime
        ...
        #print 'hello'
        #CRAZYNESS
        self.TimelinesDict['Created']['MinTime'] -= 60
        self.TimelinesDict['Created']['MaxTime'] += 60
        self.TimelinesDict['Modified']['MinTime'] -= 60
        self.TimelinesDict['Modified']['MaxTime'] += 60
        self.TimelinesDict['Accessed']['MinTime'] -= 60
        self.TimelinesDict['Accessed']['MaxTime'] += 60
        
        for key in Constants.TimelineKeys:
            self.TimelinesDict[key]['FromTime'] = self.TimelinesDict[key]['MinTime']
            self.TimelinesDict[key]['ToTime'] = self.TimelinesDict[key]['MaxTime']
        
        self.initdone = 1
        self.draw()
        self.SelectedFiles = []
        self.fileCategoryView = FileCategoryView(self, self.treeCatList, Globals.MimeTypesDict)
    ...
    def CreateTimelinesPanel(self):
        ...
        self.FileImage = GetGIF('TimelineImages/FileImage.gif')
