...
class MDIChildImages(wx.MDIChildFrame):
    ...
    def InitMenuBar(self):
        ...
        self.viewMenu.AppendCheckItem(self.IDViewHash, "Hash &Digests")
    ...
    def InitToolBar(self):
        ...
        self.toolBarMain.AddLabelTool(self.searchUpID, "SearchUp", wx.Bitmap(u'./Images/SearchUp.gif', wx.BITMAP_TYPE_GIF), shortHelp="Search Up")
        ...
        self.toolBarMain.AddLabelTool(self.searchDownID, "SearchDown", wx.Bitmap(u'./Images/SearchDown.gif', wx.BITMAP_TYPE_GIF), shortHelp="Search Down")
        ...
        tool = self.toolBarMain.AddCheckLabelTool(self.hexViewID, "HexView", wx.Bitmap(u'./Images/HexView.gif', wx.BITMAP_TYPE_GIF),
        ...
        tool = self.toolBarMain.AddCheckLabelTool(self.textViewID, "TextView", wx.Bitmap(u'./Images/TextView.gif', wx.BITMAP_TYPE_GIF),
        ...
        tool = self.toolBarMain.AddCheckLabelTool(self.nativeViewID, "NativeView", wx.Bitmap(u'./Images/NativeView.gif', wx.BITMAP_TYPE_GIF),
    ...
    def CreateHexViewWindow(self):
        ...
        self.hexEditor = hexedit.HexEdit()
        self.hexView.setModel(self.hexEditor)
        self.hexEditor.attach(self)
    ...
    def LoadFileProperties(self):
        iconFound = False
        if not (self.fileInfo.Name.rfind('.') == -1):
            fileExtension = self.fileInfo.Name[self.fileInfo.Name.rfind('.'):]
        ...
        if not iconFound:
            ...
            self.bitmapIcon.SetBitmap(bmp)
        
        self.lblFileName.SetLabel(PlatformMethods.Convert(self.fileInfo.Name))
        
        index = self.listCtrlFileProperties.InsertStringItem(sys.maxint, "Type")
        self.listCtrlFileProperties.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.MimeType))
        
        index = self.listCtrlFileProperties.InsertStringItem(sys.maxint, "Location")
        self.listCtrlFileProperties.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.DirectoryPath))
        
        index = self.listCtrlFileProperties.InsertStringItem(sys.maxint, "Size")
        self.listCtrlFileProperties.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.Size))
        
        index = self.listCtrlFileProperties.InsertStringItem(sys.maxint, "Description")
        self.listCtrlFileProperties.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.Description))
        
        index = self.listCtrlFileProperties.InsertStringItem(sys.maxint, "Created")
        self.listCtrlFileProperties.SetStringItem(index, 1, CommonFunctions.GetShortDateTime(self.fileInfo.CreatedTime))
        index = self.listCtrlFileProperties.InsertStringItem(sys.maxint, "Modified")
        self.listCtrlFileProperties.SetStringItem(index, 1, CommonFunctions.GetShortDateTime(self.fileInfo.ModifiedTime))
        index = self.listCtrlFileProperties.InsertStringItem(sys.maxint, "Accessed")
        self.listCtrlFileProperties.SetStringItem(index, 1, CommonFunctions.GetShortDateTime(self.fileInfo.AccessedTime))
        
        index = self.listCtrlHashValues.InsertStringItem(sys.maxint, "MD5")
        self.listCtrlHashValues.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.MD5Digest))
        
        index = self.listCtrlHashValues.InsertStringItem(sys.maxint, "SHA1")
        self.listCtrlHashValues.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.SHA1Digest))
        
        index = self.listCtrlHashValues.InsertStringItem(sys.maxint, "SHA224")
        self.listCtrlHashValues.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.SHA224Digest))
        index = self.listCtrlHashValues.InsertStringItem(sys.maxint, "SHA256")
        self.listCtrlHashValues.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.SHA256Digest))
        
        index = self.listCtrlHashValues.InsertStringItem(sys.maxint, "SHA384")
        self.listCtrlHashValues.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.SHA384Digest))
        index = self.listCtrlHashValues.InsertStringItem(sys.maxint, "SHA512")
        self.listCtrlHashValues.SetStringItem(index, 1, PlatformMethods.Convert(self.fileInfo.SHA512Digest))
        
        
        filePath = os.path.join(PlatformMethods.Convert(self.fileInfo.DirectoryPath), PlatformMethods.Convert(self.fileInfo.Name))
    ...
    def OpenFileInHexView(self, path, changedir=False):
        try:
            self.hexEditor.load(path, readonly=True)
        except IOError, e:
            wx.MessageBox(
    ...
    def DoSearch(self, findString, forward=True):
        ...
        try:
            ...
            wx.MessageBox(sys.exc_info()[0], "Error", wx.ICON_ERROR) 
    ...
    def OnHexPopupMenuItemSaveSelection(self, event):
        if self.hexView.selection_start is not None and self.hexView.selection_end is not None:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                #copy blocks
                t_offset = 0
                for offset, block in self.hexEditor.bin.blockReader(selection_start, 1024*1024, endoffset=selection_end):
...
class WindowHolder(wx.MDIParentFrame):
    def __init__(self, prnt, id, title, CentralID = ""):
        ...
        self.imageViewer = MDIChildImages(self)
