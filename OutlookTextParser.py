...
class OutlookTextParser():
    def __init__(self, db, AttachmentsDict, Stopwords=[], Stemmer=None, bloomFilter=None, logFile=None):
        ...
        self.textParser = TextParser.TextParser(db, Stopwords, Stemmer=self.Stemmer, bloomFilter=self.bloomFilter)
        #self.WordDict = {}
        
        
        
    def parse(self, filePath, indexMessages):
        self.filePath = filePath
        ...
        if len(filePathList) >= 2:
            ...
            self.logFile.write('File found without date time: %s\n'%(PlatformMethods.Encode(self.filePath)))
        
        #if indexMessages:
        self.lines = self.fin.readlines()
        #else:
        #    self.lines = self.fin.readlines(20)
        ...
        while self.lines:
            #for line in lines:
             #if not self.lines: 
             #   if indexMessages:
             #       self.lines = self.fin.readlines(10000)
               
            line = PlatformMethods.Encode(self.lines.pop(0)).strip()

            #else:
            #        self.lines = self.fin.readlines(20)
            #docID, data, filePath, startTime, logFile, location=0, emailPhone=False, PhoneDict={}
            if indexMessages:
                self.location = self.textParser.parse(self.DocID, line, self.filePath, time.time(), logFile=self.logFile, location=self.location, emailPhone=True, PhoneDict=self.Phones)
                
            if headerDone:
                if not indexMessages:
                    break
                ...
                if matched:
                    ...
                    if fromPart:
                        ...
                        if lastCNIndex > 0:
                            ...
                            #possiblilites
                            #From: <reception@abc.org>
                            #From: <Tom Savage>
                            #From: "Tom Savage" <tom@abc.com>
                        ...
                        self.logFile.write("No From: second part: line =  %s\n"%PlatformMethods.Encode(line))
                    ...
                    for attachment in attachList:
                        if attachment:
                            ...
                            if self.AttachmentsDict.has_key(dateTimefileName):
                                ...
                                self.logFile.write('No attachment file found for: %s in Message: %s\n'%(PlatformMethods.Encode(attachment), PlatformMethods.Encode(self.filePath)))
                    continue
        ...
        if self.fromEmail:
            ...
            """  
            manyValues = []
            if not self.toEmails:
                manyValues.append((self.DocID, self.fromEmail, 'N/A', self.sentReceivedDate, self.subject, self.attachments, self.filePath, self.attachmentsPaths, len(self.toEmails), 0, 0, 'N/A'))
            ...
            for email in self.toEmails:
                if email:
                    ...
                    if Globals.EmailsDict[self.fromEmail.lower()].has_key(email.lower()):
                        ...
                        Globals.EmailsDict[self.fromEmail.lower()][email.lower()] = {'Sent' : 1}
                    """ 
                    manyValues.append((self.DocID, self.fromEmail, email, self.sentReceivedDate, self.subject, self.attachments, self.filePath, self.attachmentsPaths, len(self.toEmails), 0, 0, 'N/A'))    
    ...
    def handleToCcBccSecondPart(self, matched):
        ...
        while moreTo:
            if self.lines:
                ...
                #self.lines = self.fin.readlines(100)
                #if not self.lines:
                break
